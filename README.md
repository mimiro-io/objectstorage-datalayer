...
`decode.listValueColumns` | list of object keys that should be interpreted as list values. The value of each key is the separateor used to split the raw value. The split values will be added to the entity as an array.
...
`decode.columnConcats` | list of object keys that should be concatenated. The value of each key is the list of fields to be concatenated. ',' is used as a seperator. The concatenation occurs before any prefixes or mappings are applied.
